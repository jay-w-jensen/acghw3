...
void GLCanvas::initialize(ArgParser *_args) {
	...
	window = glfwCreateWindow(args->width, args->height, "OpenGL viewer", NULL,
	NULL);
	if (!window) {
...
void GLCanvas::initializeVBOs() {
	...
	GLCanvas::LightID = glGetUniformLocation(GLCanvas::programID,
	...
	GLCanvas::wireframeID = glGetUniformLocation(GLCanvas::programID,
			"wireframe");
	GLCanvas::colormodeID = glGetUniformLocation(GLCanvas::programID,
			"colormode");
	// FIXME: texture still buggy
	GLCanvas::mytexture = glGetUniformLocation(GLCanvas::programID,
			"mytexture");
...
void GLCanvas::drawVBOs(const glm::mat4 &ProjectionMatrix,
	...
	glUniformMatrix4fv(GLCanvas::ModelMatrixID, 1, GL_FALSE,
			&ModelMatrix[0][0]);
	...
	if ((action == GLFW_PRESS || action == GLFW_REPEAT) && key < 256) {
		...
		case 'V':
			...
			args->render_mode = RENDER_MODE(
					(args->render_mode + 1) % NUM_RENDER_MODES);
			switch (args->render_mode) {
			case RENDER_MATERIALS:
				std::cout << "RENDER_MATERIALS\n";
				fflush(stdout);
				break;
			case RENDER_LIGHTS:
				std::cout << "RENDER_LIGHTS\n";
				fflush(stdout);
				break;
			case RENDER_UNDISTRIBUTED:
				std::cout << "RENDER_UNDISTRIBUTED\n";
				fflush(stdout);
				break;
			case RENDER_ABSORBED:
				std::cout << "RENDER_ABSORBED\n";
				fflush(stdout);
				break;
			case RENDER_RADIANCE:
				std::cout << "RENDER_RADIANCE\n";
				fflush(stdout);
				break;
			case RENDER_FORM_FACTORS:
				std::cout << "RENDER_FORM_FACTORS\n";
				fflush(stdout);
				break;
			default:
				assert(0);
		...
		case 'Q':
			...
			break;
		default:
			std::cout << "UNKNOWN KEYBOARD INPUT  '" << (char) key << "'"
					<< std::endl;
		}
		setupVBOs();
	}
...
glm::vec3 GLCanvas::TraceRay(double i, double j) {
	...
	if (args->num_antialias_samples == 1) {
		...
		MTRand_closed rng(i * j);
		for (int z = 0; z < args->num_antialias_samples; z++) {
			double x = (i - args->width / 2.0 + rng()-.5) / double(max_d) + .5 ;
			double y = (j - args->height / 2.0 +rng()-.5) / double(max_d) + .5;
			Ray r = camera->generateRay(x, y);
			Hit hit;
			color += raytracer->TraceRay(r, hit, args->num_bounces)
					/ (float) (args->num_antialias_samples);
			RayTree::AddMainSegment(r, 0, hit.getT());
		}
	}
...
int GLCanvas::DrawPixel() {
	...
	glm::vec3 pos1 = GetPos((raytracing_x) * x_spacing,
			(raytracing_y) * y_spacing);
	glm::vec3 pos2 = GetPos((raytracing_x + 1) * x_spacing,
			(raytracing_y) * y_spacing);
	glm::vec3 pos3 = GetPos((raytracing_x + 1) * x_spacing,
			(raytracing_y + 1) * y_spacing);
	glm::vec3 pos4 = GetPos((raytracing_x) * x_spacing,
			(raytracing_y + 1) * y_spacing);

	glm::vec3 color = TraceRay((raytracing_x + 0.5) * x_spacing,
			(raytracing_y + 0.5) * y_spacing);
	...
	if (raytracer->render_to_a) {
		...
		raytracer->pixels_a.push_back(
				VBOPosNormalColor(pos1, glm::vec3(0, 0, 0),
						glm::vec4(r, g, b, 1.0)));
		raytracer->pixels_a.push_back(
				VBOPosNormalColor(pos2, glm::vec3(0, 0, 0),
						glm::vec4(r, g, b, 1.0)));
		raytracer->pixels_a.push_back(
				VBOPosNormalColor(pos3, glm::vec3(0, 0, 0),
						glm::vec4(r, g, b, 1.0)));
		raytracer->pixels_a.push_back(
				VBOPosNormalColor(pos4, glm::vec3(0, 0, 0),
						glm::vec4(r, g, b, 1.0)));
		raytracer->pixels_indices_a.push_back(
				VBOIndexedTri(start + 0, start + 1, start + 2));
		raytracer->pixels_indices_a.push_back(
				VBOIndexedTri(start + 0, start + 2, start + 3));
	} else {
		int start = raytracer->pixels_b.size();
		raytracer->pixels_b.push_back(
				VBOPosNormalColor(pos1, glm::vec3(0, 0, 0),
						glm::vec4(r, g, b, 1.0)));
		raytracer->pixels_b.push_back(
				VBOPosNormalColor(pos2, glm::vec3(0, 0, 0),
		...
		raytracer->pixels_b.push_back(
				VBOPosNormalColor(pos4, glm::vec3(0, 0, 0),
						glm::vec4(r, g, b, 1.0)));
		raytracer->pixels_indices_b.push_back(
				VBOIndexedTri(start + 0, start + 1, start + 2));
		raytracer->pixels_indices_b.push_back(
				VBOIndexedTri(start + 0, start + 2, start + 3));
	}
...
int HandleGLError(const std::string &message, bool ignore) {
	...
	while ((error = glGetError()) != GL_NO_ERROR) {
		if (!ignore) {
			...
			std::cerr << "GL ERROR(" << i << ") " << WhichGLError(error)
					<< std::endl;
		}
		i++;
	}
