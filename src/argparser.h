		...
		for (int i = 1; i < argc; i++) {
			if (std::string(argv[i]) == std::string("-input")
				...
				num_form_factor_samples = atoi(argv[i]);
					...
					== std::string("-sphere_rasterization")) {
				i++;
				assert(i < argc);
				...
				ambient_light = glm::vec3(r, g, b);
					...
					== std::string("-num_photons_to_shoot")) {
				i++;
				assert(i < argc);
